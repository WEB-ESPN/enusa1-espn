                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody v-if="datas.data.length > 0" class="tbody-">
                            <tr v-for="(data, index) in datas.data" :key="index">                                    
                                ...
                                <td>{{ data.content }}</td>
                                <td>
                                    <img :src="data.file_name" :alt="data.title" class="img" :width="300" v-if="data.isImageType">
                                    <span v-else-if="!data.isImageType && data.file_name !== null" @click.prevent="download(data.file_name)">
                                        Download File
                                    </span>
                                </td>
                                <td>
            ...
            <div class="modal-content">
                ...
                <Form ref="form" @submit="handleSubmit" :validation-schema="statusEditing ? editDataSchema : createDataSchema" v-slot="{ errors }" :initial-values="formValues">
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label for="file_name">Image/File</label>
                            <div v-if="statusEditing">
                                <br>
                                <img :src="formImage.image" class="img" :width="150">
                            </div>
                            <Field name="file_name" type="file" class="form-control" :class="{ 'is-invalid': errors.file_name }" id="file_name" />
                            <span class="invalid-feedback">{{ errors.file_name }}</span>
                        </div>
...
<script setup>
    ...
    const createDataSchema = yup.object({
        ...
        file_name  : yup.string().nullable(true),
    });
    ...
    const initForm = (data = null) => {
        if (data) {
            ...
            formImage.value = { image : data.file_name };            
        } else {
            formValues.value = {
                id         : null,
                key        : null,
    ...
    const createData = (values, { resetForm, setErrors }) => {
        ...
        .catch((error) => {
            if (error.response.status = 422) {
                setErrors({key: error.response.data.message})
            }
